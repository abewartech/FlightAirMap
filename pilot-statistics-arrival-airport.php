...
{
	$title = sprintf(_("Most Common Arrival Airports of %s"),$spotter_array[0]['pilot_name']);
...
//	print '<div><span class="label">'._("Airline").'</span><a href="'.$globalURL.'/airline/'.$spotter_array[0]['airline_icao'].'">'.$spotter_array[0]['airline_name'].'</a></div>'; 
	...
	print '<p>'.sprintf(_("The statistic below shows all arrival airports of flights piloted by <strong>%s</strong>."),$spotter_array[0]['pilot_name']).'</p>';
	$airport_airport_array = $Spotter->countAllArrivalAirportsByPilot($pilot,$filter);
	...
	foreach($airport_airport_array as $airport_item)
	{
		...
		print '</td>';
		print '<td><a href="'.$globalURL.'/search?arrival_airport_route='.$airport_item['airport_arrival_icao'].'&pilot_name='.$spotter_array[0]['pilot_name'].'">'._("Search flights").'</a></td>';
		print '</tr>';
		$i++;
	}
	print '<tbody>';
