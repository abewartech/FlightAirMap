...
class create_db {
	...
	public static function import_file($filename) {
		$filename = filter_var($filename,513);
	...
	public static function create_database($root,$root_pass,$user,$pass,$db,$db_type,$host,$port = '') {
		$root = filter_var($root,513);
		$root_pass = filter_var($root_pass,513);
		$user = filter_var($user,513);
		$password = filter_var($pass,513);
		$db = filter_var($db,513);
		$db_type = filter_var($db_type,513);
		$host = filter_var($host,513);
